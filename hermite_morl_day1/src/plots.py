...
def save_metric_curve(summary_df, output_path: str | Path) -> None:
    ...
    width, height, margin = 760, 460, 70
    ...
    draw.text((width // 2 - 120, height - 45), "Número de componentes seleccionados (k)", fill="black", font=small)
    draw.text((margin, 25), "MSE y SSIM normalizados", fill="black", font=small)
    ...
    if len(mse_points) > 1:
        draw.line(mse_points, fill="black", width=3)
        draw.line(ssim_points, fill="gray", width=3)
    for p in mse_points:
        draw.ellipse((p[0] - 4, p[1] - 4, p[0] + 4, p[1] + 4), fill="black")
    for p in ssim_points:
        draw.rectangle((p[0] - 4, p[1] - 4, p[0] + 4, p[1] + 4), fill="gray")

    draw.text((width - 200, margin), "● MSE medio (norm.)", fill="black", font=font)
    draw.text((width - 200, margin + 25), "■ SSIM medio (norm.)", fill="gray", font=font)
    for k in xs:
        x = _scale_points([k], [0], width, height, margin)[0][0]
        draw.text((x - 8, height - margin + 10), str(int(k)), fill="black", font=small)
